...
public class AuthController {
    ...
    public ResponseEntity<String> registerUser(@RequestBody User user) {
        ...
        try {
            ...
            // Step 3: Prevent NOT NULL DB Errors and fill missing optional fields
            if (user.getIncome() == null) {
                user.setIncome(0L);
            }

            // If the DB expects an 'email' column and the client didn't provide one,
            // fallback to using the username so the INSERT doesn't fail when the
            // column is declared NOT NULL in the database.
